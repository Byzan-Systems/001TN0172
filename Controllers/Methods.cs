    ...
    public class Methods
    {
        ...
        public static void Fill_CashOpsDetails_Manual(string NEFT_RTGS_BT_ID, string[] Details, string[] detailsCash, ILogger logger)
        {
            ...
            catch (Exception ex)
            {
                ...
                string sanitizedAccountNo = accountno.Replace(Environment.NewLine, "").Replace("\n", "").Replace("\r", "");
                string sanitizedUTR = detailsCash[5].Replace(Environment.NewLine, "").Replace("\n", "").Replace("\r", "");
                logger.LogError("Exception occurred: {Message}. VirtualAccount ending: {VirtualAccountMasked}, UTR Present: {UTRPresent}", ex.Message, sanitizedAccountNo, !string.IsNullOrWhiteSpace(sanitizedUTR));
